...
try {
    switch ($method) {
        case 'GET':
            requireApiAuth();
            if ($id) {
                $data = ordonnanceService_getById($id);
                if (!$data) {
                ...
                http_response_code(200);
                echo json_encode(ordonnanceService_getAll());
            }
            break;
        case 'POST':
            requireApiRole(['admin', 'medecin']);
            ordonnanceService_create(
        ...
        case 'PUT':
            ...
            ordonnanceService_update($id, $_POST['medicaments'] ?? '');
            http_response_code(200);
